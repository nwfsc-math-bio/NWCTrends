...
for (i in 1:(dim(vals1)[2] - 1)) {
  ...
  points(i + rnorm(n, 0, 0.01), ifelse(vals < 0, -1, 1) * log10(abs(vals)),
    cex = 1.5, pch = 19, 
    col = ifelse(vals < 0, nwctrends.palette$red, nwctrends.palette$black)
  )
}
...
x <- barplot(vals, xaxt = "n", 
             col = c(rep(nwctrends.palette$red, 6), 
                     rep(nwctrends.palette$black, 6)), ylab = "Num of populations")
...
plot(rep(1, dim(a)[1]), vals[[1]],
  ...
  col = ifelse(a[, 3] < 0, nwctrends.palette$red, nwctrends.palette$black),
...
if (nrange > 1) {
  for (i in 2:nrange) {
    points(rep(i, dim(a)[1]), vals[[i]], 
           col = ifelse(a[, yr.start + i - 1] <= 0, nwctrends.palette$red, nwctrends.palette$black), cex = 1.5, pch = 19)
  }
}
abline(h = 0)
