...
coho.chum=any(str_detect(pops.to.plot.wild[1],c("Coho","Chum","coho", "chum")))
...
barplot(table(cut(as.numeric(as.character((a[,6]))),seq(0,1,.2)),exclude=NULL), 
        names.arg=c(as.character(seq(0.1,.9,0.2)),"NA"),ylab="Count",
        ylim=c(0,dim(a)[1]))
...
for(i in 1:dim(a)[1]){
  vals1=rbind(vals1, as.numeric(apply(a[i,geo.start:geo.end],2,function(x){str_split(x," [(]")[[1]][1]})))
  vals=apply(a[i,geo.start:geo.end],2,function(x){str_split(x," [(]")[[1]][2]})
  vals=sapply(vals,function(x){str_split(x,"[)]")[[1]][1]})
...
vals=sapply(a[,"% Change"],function(x){str_split(x," [(]")[[1]][1]})
...
vals1 = as.numeric(sapply(a[,3],function(x){str_split(x," [(]")[[1]][1]}))
vals2 = as.numeric(sapply(a[,4],function(x){str_split(x," [(]")[[1]][1]}))
...
print(xtable(a[[1]], align="rr|c|cccccc", digits=0), 
      ...
      file="figures/geomean_total_table.tex")
...
if(output.type=="latex") cat("\\begin{landscape}")
...
print(xtable(a, align="rr|ccccc", digits=2), 
      comment=FALSE, include.rownames=FALSE, table.placement="p", type=output.type,
      file="figures/fracwild_table.tex")
...
print(xtable(a, align="rr|c|cc", digits=2), 
      ...
      file="figures/trend_15_table.tex") 
